...
It is clear from the plots that the SVM with a radial kernel provides the best fit on the training data where as the Support Vector Classifier (SVC) fails entirely to separate the two classes. The SVM with a polynomial kernel does a slightly better job than the SVC but only just. This is refelcted in the training error rates reported as SVC - 25.33%, SVM(poly) - 22.67% and SVM(radial) - 9.33%.

When it comes to the test error rates, the SVM(poly) does no better than SVC. The test error rate of SVM(radial) are the best of the lot at 12%.
